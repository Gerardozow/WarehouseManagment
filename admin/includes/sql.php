...
function authenticate($username = '', $password = '')
{
  ...
  // Convertir el nombre de usuario a minúsculas antes de buscar en la base de datos
  $username_lowercase = strtolower($username);
  $sql  = sprintf("SELECT id,username,password,user_level FROM users WHERE LOWER(username) = '%s' LIMIT 1", $username_lowercase);
  $result = $db->query($sql);
